 ...
 *
 * ----------	---------	-------------------------------------	--------------------------
 * 2014-03-20	guofeizhi	support LCM dev_info inquiry		FEIXUN_SUPPORT_LCM_DEVINFO_INQUIRY_GUOFEIZHI_001
 *
 */
...
#define BOE_OTM8018_ID		0x8018
#define DESC_BOE_OTM8018	"LCD_BOE_OTM8018B_Video"
#define FEIXIAN_FL10801A_ID	0x1080
#define DESC_FX_FL10801A	"LCD_FX_FL10801A_Video"
#else
#define APEX_ILI9806C_ID	0x9806
#define DESC_ILI9806C		"lcd_apex_ili9806c_video"
#define FEIXUN_FL10801A_ID	0x1080
#define DESC_FL10801A		"lcd_feixun_fl10801a_video"
#define FEIXUN_NT35510_ID	0x5510
#define DESC_NT35510		"lcd_feixun_NT35510_cmd"
#endif
#endif
...
#ifdef CONFIG_DEVICE_VERSION
//FEIXUN_SUPPORT_LCM_DEVINFO_INQUIRY_GUOFEIZHI_001 start
#if defined CONFIG_PHICOMM_BOARD_E550W
	register_fdv_with_desc(DEV_LCD, MANUF_BOE, MANUF_BOE_ID|BOE_OTM8018_ID, DESC_BOE_OTM8018);
	register_fdv_with_desc(DEV_LCD, MANUF_FEIXIAN, MANUF_FEIXIAN_ID|FEIXIAN_FL10801A_ID, DESC_FX_FL10801A);

	if(ctrl_pdata->panel_data.panel_info.panel_name == NULL) {
		pr_err("%s: can't find panel name \n", __func__);
	} else {
		if (!strncmp(ctrl_pdata->panel_data.panel_info.panel_name, "OTM8018b", 8)) {
			confirm_fdv(DEV_LCD, MANUF_BOE, MANUF_BOE_ID|BOE_OTM8018_ID);
		} else if (!strncmp(ctrl_pdata->panel_data.panel_info.panel_name, "fl10801a", 8)) {
			confirm_fdv(DEV_LCD, MANUF_FEIXIAN, MANUF_FEIXIAN_ID|FEIXIAN_FL10801A_ID);
		} else {
			;
		}
	}
#else
//FEIXUN_SUPPORT_LCM_DEVINFO_INQUIRY_GUOFEIZHI_001 end
	register_fdv_with_desc(DEV_LCD, MANUF_APEX, MANUF_APEX_ID|APEX_ILI9806C_ID, DESC_ILI9806C);
	register_fdv_with_desc(DEV_LCD, MANUF_FEIXUN, MANUF_FEIXUN_ID|FEIXUN_FL10801A_ID, DESC_FL10801A);
	register_fdv_with_desc(DEV_LCD, MANUF_FEIXUN, MANUF_FEIXUN_ID|FEIXUN_NT35510_ID, DESC_NT35510);

	if(ctrl_pdata->panel_data.panel_info.panel_name == NULL) {
		pr_err("%s: can't find panel name \n", __func__);
	} else {
		if (!strncmp(ctrl_pdata->panel_data.panel_info.panel_name, "ili9806c", 8)) {
			confirm_fdv(DEV_LCD, MANUF_APEX, MANUF_APEX_ID|APEX_ILI9806C_ID);
		} else if (!strncmp(ctrl_pdata->panel_data.panel_info.panel_name, "fl10801a", 8)) {
			confirm_fdv(DEV_LCD, MANUF_FEIXUN, MANUF_FEIXUN_ID|FEIXUN_FL10801A_ID);
		} else {
			confirm_fdv(DEV_LCD, MANUF_FEIXUN, MANUF_FEIXUN_ID|FEIXUN_NT35510_ID);
		}
	}
#endif
#endif
