...
#define DESC            "akm_compass_sensor" 
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk( KERN_ERR "wqc:akm irq: %d\n", s_akm->irq );
    if( s_akm->irq == 0 ) {
        s_akm->irq = client->irq = 83;
    }
...
#ifdef CONFIG_DEVICE_VERSION
	confirm_fdv(DEV_GEOMAGNETIC,MANUF_AKM,MANUF_AKM_ID|0x0d);
#endif
	return 0;
...
#ifdef CONFIG_DEVICE_VERSION
	register_fdv_with_desc(DEV_GEOMAGNETIC,MANUF_AKM,MANUF_AKM_ID|0x0d,DESC);
#endif
